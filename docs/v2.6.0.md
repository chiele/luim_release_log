...
#### 6️⃣  新增财务-数据导出-支付宝流水数据导出，可选择支付宝账务明细导出（支付宝账单接口中匹配到淘宝订单号的原始数据）、支付宝账务总金额导出（以淘宝订单做唯一项，计算当前订单下的支付宝流水总和）、支付宝账务备注导出（匹配不到对应淘宝订单号的数据，例如转账等），可选择多选或全选（多选后文档以sheet分页的形式展出），并且筛选时间（导出时长最长不超过一个月）。

![7](https://luim-public.oss-cn-zhangjiakou.aliyuncs.com/release_docs/v2.6.0/7.png) 

#### 7️⃣  新增财务-数据导出-已卖出宝贝数据导出，可选择单一店铺或者全选（多选后文档以sheet分页的形式展出），并筛选时间后导出（导出时长最长不超过一个月）。
